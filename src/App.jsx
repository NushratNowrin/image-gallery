...
function App() {
  ...
  const moveImage = (dragIndex, hoverIndex) => {
    ...
    setChecked((prevChecked) => {
      const updatedChecked = prevChecked.filter((_, index) => index !== dragIndex);
  ...
  const ImageItem = ({ index, image }) => {
    ...
    return (
      <div ref={(node) => drag(drop(node))} className={`gallery-img ${index === 0 ? "col-span-2 row-span-2" : ""}`} draggable>
        <div
